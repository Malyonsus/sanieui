--[[
Create the ToT unit frame.
--]]

if(SanieUI.debug == true) then
	print("Loading ToT data")
end

local uf = SanieUI.unitframes

local height = uf.targettarget.height
local width = uf.targettarget.width
local scale = uf.targettarget.scale
local texture = uf.bartexture
local xpos = uf.targettarget.xpos
local ypos = uf.targettarget.ypos
local backdropmargin = uf.backdropmargin

local hpweight = uf.targettarget.hpweight
local ppweight = uf.targettarget.ppweight
...
oUF:RegisterStyle("TargetTarget", CreateTargetStyle)
oUF:SetActiveStyle("TargetTarget")
oUF:Spawn("targettarget", "oUF_Sanie_TargetTargetFrame")